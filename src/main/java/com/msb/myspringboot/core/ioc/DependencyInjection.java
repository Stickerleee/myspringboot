...
import com.msb.myspringboot.exception.InterfaceNotExistsImplementException;
import com.msb.myspringboot.exception.NotFountTargetBeanException;
	...
	public static void loadDependency(Object bean) {
		...
		for (Field field : bean.getClass().getDeclaredFields()) {
			...
			if (field.isAnnotationPresent(Autowired.class)) {
				...
				// 判断是否是接口 如果是接口则需要顺延找到实现类
				if (fieldTypeClass.isInterface()) {
					Set<Class<?>> subClass = ReflectionUtil.getSubClass(
							ApplicationContext.getInstance().getPackageNames(), (Class<Object>) fieldTypeClass);
