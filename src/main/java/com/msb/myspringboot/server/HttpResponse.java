package com.msb.myspringboot.server;

import static io.netty.handler.codec.http.HttpResponseStatus.INTERNAL_SERVER_ERROR;

import com.msb.myspringboot.entity.ErrorResponse;
import com.msb.myspringboot.serialize.impl.JacksonSerializer;
...
public class HttpResponse {
	...
	public static FullHttpResponse internalServerError(String uri) {
		ErrorResponse errorResponse = new ErrorResponse(INTERNAL_SERVER_ERROR.code(),
				INTERNAL_SERVER_ERROR.reasonPhrase(), uri);
		byte[] bytes = JACKSON_SERIALIZER.serialize(errorResponse);
		DefaultFullHttpResponse response = new DefaultFullHttpResponse(HttpVersion.HTTP_1_1, INTERNAL_SERVER_ERROR,
				Unpooled.wrappedBuffer(bytes));
